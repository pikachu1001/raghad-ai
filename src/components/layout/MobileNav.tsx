...
export function MobileNav({ className = "" }: { className?: string }) {
  ...
  const items: NavItem[] = [
    ...
    { href: "/about#vision", label: messages.nav.vision },
  ...
  const drawer = open && mounted ? (
    createPortal(
      <>
        ...
        <aside
          dir={dir}
          className="fixed inset-y-0 start-0 z-[210] flex w-[min(100%,20rem)] flex-col bg-[#faf6ef] shadow-2xl ring-1 ring-[#c9a962]/20"
        >
          <div className="flex items-center justify-between gap-3 border-b border-[#ddd0b8]/50 px-5 py-4">
            <div className="flex min-w-0 items-center gap-2.5">
              <BrandLogo size="sm" />
              <span className="luxury-heading-section truncate text-lg">
                {messages.brand}
              </span>
            </div>
            <button
          ...
          <div className="border-t border-[#ddd0b8]/50 px-5 py-5">
